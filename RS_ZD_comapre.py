...
try:
    ...
    depth = (depth_image - depth_image.min()) / (depth_image.max() - depth_image.min()) * 255.0
    ...
    pred_crop = pred_im[centre[0]-100:centre[0]+100, centre[1]-100:centre[1]+100]

    hist_depth = cv2.calcHist([depth_crop], [0], None, [np.max(depth_crop)], [0,np.max(depth_crop)])
    # hist_pred = cv2.calcHist([pred_crop], [0], None, [np.max(pred_crop)], [0,np.max(pred_crop)])
